describe('Validações de Alertas em JavaScript', () => {
    ...
    it('Deve interagir com um prompt, inserir um texto e validar uma mensage', () =>{
        cy.window().then((win) => {
            cy.stub(win, 'prompt').returns('Maués')
        })

         cy.on('window:alert',(msg) => {
            expect(msg).to.equal('Olá Maués! Boas-vindas ao WebDojo!')
        })
